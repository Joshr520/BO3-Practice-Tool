...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness_draw)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Separator), thickness_draw);
            if (g.LogEnabled)
...
void ImGui::Separator(float thickness_draw)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    SeparatorEx(flags, thickness_draw);
}
