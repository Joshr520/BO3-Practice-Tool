StalingradSoftPatch()
{
    ...
    while((self.bgb_pack_randomized[0] != "zm_bgb_reign_drops" && self.bgb_pack_randomized[0] != "zm_bgb_shopping_free") || (self.bgb_pack_randomized[3] != "zm_bgb_anywhere_but_here" && self.bgb_pack_randomized[4] != "zm_bgb_anywhere_but_here"))
    {
        self.bgb_pack_randomized = array::randomize(self.bgb_pack);
    }
    level flag::wait_till("initial_blackscreen_passed");
...
IncreaseBoxOdds(weapons)
{
	...
	if((randomint(100) <= n_chance || level.var_12d3a848 >= 5) && zm_magicbox::treasure_chest_canplayerreceiveweapon(self, level.w_raygun_mark3, var_b45fbf8c) && !self HasWeapon(level.w_raygun_mark3_upgraded))
	{
